    ...
    <div className="h-full flex flex-col gap-4 min-h-0">
      ...
      <div className="grid grid-cols-3 gap-4 flex-1 min-h-0">
        ...
        <div className="col-span-2 flex flex-col gap-4 min-h-0">
          ...
          {/* Ingresos mensuales — fondo oscuro */}
          <div className="flex-1 min-h-0 flex flex-col rounded-2xl p-5 overflow-hidden relative"
            style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%)', boxShadow: '0 8px 24px rgba(15,23,42,0.25)' }}>
            <div className="absolute -right-8 -top-8 w-32 h-32 rounded-full bg-white/5" />
            <div className="absolute right-4 bottom-4 w-16 h-16 rounded-full bg-white/5" />
            <h2 className="text-sm font-semibold text-white/80 mb-3 shrink-0 relative">Ingresos mensuales</h2>
            {incomeChartData.length === 0 ? (
              <div className="flex-1 flex items-center justify-center text-white/30 text-sm">Sin datos aún</div>
            ) : (
              <div className="flex-1 min-h-0 relative">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={incomeChartData}>
                    ...
                    <YAxis tick={{ fontSize: 10, fill: 'rgba(255,255,255,0.5)' }} tickFormatter={v => `$${(v/1000).toFixed(0)}k`} width={45} axisLine={false} tickLine={false} />
                    <Tooltip contentStyle={{ background: '#1e293b', border: 'none', borderRadius: 12, color: 'white' }} formatter={(v) => [`$${v.toLocaleString('es-CO')}`, 'Ingresos']} />
                    <Line type="monotone" dataKey="ingreso" stroke="#60a5fa" strokeWidth={2.5} dot={{ fill: '#60a5fa', r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
        ...
        <div className="flex flex-col gap-4 min-h-0">
          ...
          {/* Top clientes — fondo azul medio */}
          <div className="flex-1 min-h-0 flex flex-col rounded-2xl p-5 relative overflow-hidden"
            ...
            <h2 className="text-sm font-semibold text-white/80 mb-3 shrink-0 relative">Top clientes</h2>
            {topCustomers.length === 0 ? (
              <div className="flex-1 flex items-center justify-center text-white/40 text-sm">Sin datos</div>
            ) : (
              <div className="space-y-2 overflow-y-auto flex-1 relative">
                {topCustomers.slice(0, 5).map((c, i) => (
                  <div key={c.customerId} className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-white/10">
                    <span className="w-5 h-5 rounded-full bg-white/20 text-white text-xs font-bold flex items-center justify-center shrink-0">
                      {i + 1}
                    </span>
                    <span className="flex-1 text-sm text-white/90 truncate">{c.customerName}</span>
