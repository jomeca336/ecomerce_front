...
export default function DashboardHome() {
  ...
  const stats = [
    ...
    { label: 'Productos', value: products.length.toLocaleString('es-CO'), sub: `${products.filter(p => p.active).length} activos`,
      ...
      grad: 'from-blue-500 to-indigo-700' },
  ]
      ...
      <div className="grid grid-cols-4 gap-4 shrink-0">
        {stats.map(s => (
          <div key={s.label} className="card p-4 flex items-center gap-4">
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${s.grad} flex items-center justify-center shrink-0 shadow`}>
              ...
              <div className="flex-1 min-h-0">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={incomeChartData}>
                    ...
                    <YAxis tick={{ fontSize: 10, fill: '#9ca3af' }} tickFormatter={v => `$${(v/1000).toFixed(0)}k`} width={45} />
                    <Tooltip formatter={(v) => [`$${v.toLocaleString('es-CO')}`, 'Ingresos']} />
                    <Line type="monotone" dataKey="ingreso" stroke="#1d4ed8" strokeWidth={2.5} dot={{ fill: '#1d4ed8', r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
